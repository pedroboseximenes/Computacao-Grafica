export class Vertex {
  ...
  export class HalfEdgeDS {
    ...
    colorirMalha(vid, gl, mesh) {
      const colorGreen = [100, 128 ,0, 1];  // verde escuro
      const first = this.vertices[vid].he;
      first.vertex.color = colorGreen;
      
      let he = first.opposite;
      var distance = 1;
      for(let i = 0; he != first.opposite || i == 0; i++){
        if (i <= distance) {
          he.vertex.color = colorGreen;
          let nextHalfEdge = he.next;
          do {
              nextHalfEdge.vertex.color = colorGreen;
              nextHalfEdge = nextHalfEdge.next;
          } while (nextHalfEdge !== he.next);
